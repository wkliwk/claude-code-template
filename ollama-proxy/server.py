...
# Configure logging
logging.basicConfig(
    level=logging.INFO,  # Temporarily changed to DEBUG for troubleshooting
...
# Get Ollama endpoint if available
...
FALLBACK_CHAIN = [
    "openrouter/qwen/qwen3-coder",                        # 1. best coding model (paid, full throughput)
    "openrouter/deepseek/deepseek-chat",                  # 2. general purpose, very cheap
    "openrouter/qwen/qwen3-coder:free",                   # 3. free fallback if paid fails
    "openrouter/nvidia/nemotron-3-super-120b-a12b:free",  # 4. large reasoning fallback
    "openrouter/openrouter/free",                         # 5. dynamic free router
    "ollama/qwen2.5-coder:7b",                            # 6. local offline fallback
    "ollama/phi3:mini",                                   # 7. ultra-light last resort
]
...
def clean_gemini_schema(schema: Any) -> Any:
    ...
    if isinstance(schema, dict):
        ...
        # Gemini doesn't support anyOf/oneOf/allOf (Union types).
        # Collapse anyOf to the first non-null type found.
...
class MessagesRequest(BaseModel):
    ...
    def validate_model_field(cls, v, info): # Renamed to avoid conflict
        ...
        # Add prefixes to non-mapped models if they match known lists
            ...
            if clean_v in OLLAMA_MODELS and not v.startswith('ollama/'):
...
def convert_litellm_to_anthropic(litellm_response: Union[Dict[str, Any], Any],
    ...
    try:
        ...
        # Add tool calls if present — convert to Anthropic tool_use format for ALL models
        if tool_calls:
            logger.debug(f"Processing tool calls for model: {clean_model}")
...
def _is_context_too_large_error(err: Exception) -> bool:
    """Return True when the error is a context-length / token-limit error."""
    msg = str(err).lower()
    return "context" in msg or "tokens" in msg or "context_length_exceeded" in msg


def _build_request_for_model(base_request: dict, model: str) -> dict:
    """Return a copy of base_request with the model field and provider credentials set."""
    req = base_request.copy()
    req["model"] = model

    # Remove any previously-set provider credentials so we set the correct ones
    req.pop("api_key", None)
    req.pop("api_base", None)

    if model.startswith("openrouter/"):
        req["api_key"] = OPENROUTER_API_KEY
    elif model.startswith("ollama/"):
        req["api_base"] = OLLAMA_API_BASE
    elif model.startswith("openai/"):
        req["api_key"] = OPENAI_API_KEY
    elif model.startswith("groq/"):
        req["api_key"] = GROQ_API_KEY
    elif model.startswith("gemini/"):
        req["api_key"] = GEMINI_API_KEY
    else:
        req["api_key"] = ANTHROPIC_API_KEY

    return req


def attempt_completion(base_request: dict, models_to_try: list) -> tuple:
    """Try each model in models_to_try in order.

    Returns (litellm_response, model_used) on first success.
    Raises the last exception if every model fails.
    """
    last_exc: Exception = RuntimeError("No models to try")
    for model in models_to_try:
        req = _build_request_for_model(base_request, model)
        try:
            response = litellm.completion(**req)
            return response, model
        except Exception as exc:
            err_str = str(exc)
            if _is_context_too_large_error(exc):
                logger.warning(f"⚠ {model} failed (context too large), trying next...")
            else:
                logger.warning(f"⚠ {model} failed ({err_str[:120]}), trying next...")
            last_exc = exc
    raise last_exc


async def async_attempt_completion(base_request: dict, models_to_try: list) -> tuple:
    """Async version of attempt_completion for streaming path.
...
async def create_message(
    ...
    try:
        ...
        if request.model.startswith("openai/"):
            ...
            logger.debug(f"Using Ollama endpoint for model: {request.model} at {OLLAMA_API_BASE}")
        else:
        ...
        if request.stream:
            ...
            # Build the ordered list of models to try: primary first, then fallbacks (deduped)
            primary_model = litellm_request.get("model", "")
            models_to_try = [primary_model] + [m for m in FALLBACK_CHAIN if m != primary_model]
            response_generator, stream_model_used = await async_attempt_completion(litellm_request, models_to_try)
            if stream_model_used != primary_model:
                logger.info(f"✓ Used fallback (streaming): {stream_model_used}")
            ...
            primary_model = litellm_request.get("model", "")
            models_to_try = [primary_model] + [m for m in FALLBACK_CHAIN if m != primary_model]
            litellm_response, model_used = attempt_completion(litellm_request, models_to_try)
            if model_used != primary_model:
                logger.info(f"✓ Used fallback: {model_used}")
            else:
                logger.debug(f"✅ RESPONSE RECEIVED: Model={model_used}, Time={time.time() - start_time:.2f}s")
